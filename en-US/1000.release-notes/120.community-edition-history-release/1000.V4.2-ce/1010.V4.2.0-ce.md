...
|slug|odp-community-edition-v4-2-0|
...
* Fixed the issue where the cluster is disconnected when the `PREPARE st1 from @A` statement is executed.

* Fixed the issue where the behavior in random routing is not as expected.
...
* Fixed the issue where disconnection occurs due to excessively long user variables in a PL statement.
