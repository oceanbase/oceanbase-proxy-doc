...
OceanBase Cloud supports geo-disaster recovery for the clusters where primary and standby tenants reside. However, it requires you to switch between domain names in scenarios other than database switchover, which is less flexible compared to ODP for off-cloud users. As more on-cloud users adopt primary and standby tenants, capabilities of this feature must be improved to support read/write splitting and primary/standby switchover with one connection string.

ODP V4.3.3 supports read/write splitting between the primary and standby tenants, allowing automatic login to local clusters where the primary and standby tenants reside and automatic read/write splitting between these clusters without modifying the login connection string. To reduce the cross-region bandwidth costs, a compression protocol is used between ODP and OceanBase Database.
...
* ODP V4.3.3 supports accurate routing for more expressions, such as TO_DAYS(), TIMESTAMP 'xxxxx', and NOT IN.
...
* PS cache monitoring
  ...
  In a scenario where the prepared statement (PS) protocol is used, a client may keep a large amount of PS caches in use, which occupies a large amount of memory and degrades the performance. It is difficult to diagnose such an issue. To improve troubleshooting efficiency for PS protocol-related issues, ODP provides PS cache information, such as the number of allocated PS caches and memory usage, in Prometheus monitoring and periodically outputs the monitoring data. It also allows you to execute the `SHOW PROXYPS` statement to query information in the PS cache, such as the SQL text, parsing results, and memory usage of prepared statements, to improve the usability of diagnostics.
...
|-----------------|-----------|---------------------------------------------------------------------------------------------|
|log_bin_basename|New        |ODP can forward this variable to the binlog service to query the prefix of binlog file names.|
...
ODP V4.3.3 provides multiple new features. We recommend that you use ODP V4.3.3 if your new environment relies on the new features, or the LTS version V4.3.1 if otherwise.
