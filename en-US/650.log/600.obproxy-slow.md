...
* Log format
  ...
  * The `SQL` field varies based on the type of request executed. The details are as follows:

    * **SQL request**: The executed SQL statement is printed.
    * **OBKV request**: The routing information of the OBKV request is printed.

    The routing information of an OBKV request is in the `{request_type}-{table_id}-{partition_id}-{ignorable}-{ignorable}` format, where the fields are defined as follows:

    * `{request_type}`: The type of the OBKV request. Valid values: `shard`, `sub`, and `single`.
      * `shard`: the parent request in a cross-partition request.
      * `sub`: the child request in a cross-partition request.
      * `single`: a non-cross-partition request.
    * `{table_id}`: the unique ID of the table.
    * `{partition_id}`: the ID of the partition to which the request is routed. If the request is a parent request in a cross-partition request, the value is `-1`.
    * `{ignorable}`: the value is fixed to `0`. You can ignore this field.
...
For more information about the `slow_query_time_threshold` parameter, see [slow_query_time_threshold](../400.configuration-management/300.multi-level-configuration-items/2350.slow-query-time-threshold.md).
