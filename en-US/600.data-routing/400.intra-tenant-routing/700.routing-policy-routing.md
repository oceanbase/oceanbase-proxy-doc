...
* TARGET_REPLICA_TYPE_WITH_LEADER: Read requests are routed to specified replicas. In this strategy, the leader and followers have the same priority. The replica type is specified by `route_target_replica_type`.
...
For more information about `route_target_replica_type`, see [route_target_replica_type](../../400.configuration-management/300.multi-level-configuration-items/2040.route_target_replica_type.md). For more information about `weakread_weight_zone`, see [weakread_weight_zone](../../400.configuration-management/300.multi-level-configuration-items/2600.weakread-weight-zone.md).

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <ul>
  <li><p>When <code>route_target_replica_type</code> is set to <code>ColumnStore</code>, ODP routes read requests only to columnstore replicas instead of the leader. </p></li>
  <li><p>The routing strategies related to read-only zones have been deprecated. </p></li>
  </ul>
</main>
...
3. Configure the instance-level parameter `proxy_route_policy` of ODP for more fine-grained priority control. If you only need to control the routing to a zone or IDC, you can skip this configuration step. If this parameter is not configured, ODP uses the default routing strategy `MERGE_IDC_ORDER`.
...
2. Execute the EXPLAIN ROUTE statement to view the routing process in ODP.
...
<main id="notice" type='explain'>
   <h4>Note</h4>
   <p>You can configure the <code>proxy_idc_name</code> parameter to route weak-consistency read requests to another region. For example, you can set <code>proxy_idc_name</code> to `yue` to route weak-consistency read requests to HANGZHOU. </p>
</main>
