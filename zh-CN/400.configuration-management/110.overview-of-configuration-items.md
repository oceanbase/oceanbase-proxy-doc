...
| [refresh_config](200.dynamically-effective/1960.refresh-config.md) | 用于控制是否在表处理器进行检查工作时更新所有代理配置。  |
...
| [cache_cleaner_clean_interval](200.dynamically-effective/90.cache-cleaner-clean-interval%20%20.md) | 用于设置 cache cleaner 清理缓存的间隔时间。 |
| [server_state_refresh_interval](200.dynamically-effective/2130.server-state-refresh-interval.md) | 用于设置刷新 OceanBase 数据库状态的间隔时间，用于获取Zone 或 OBServer 节点的最新状态。 |
...
<!-- cluster_count_high_water_mark 为啥会归属到重复任务间隔相关 -->
...
| [observer_query_timeout_delta](200.dynamically-effective/1690.observer-query-timeout-delta%20%20.md) | 用于设置网络传输延时，为 OceanBase 数据库的 ob_query_timeout 设置的增量。 |
...
| 配置项  |  功能描述    |
|---------|--------------|
| [frequent_accept](300.restart-takes-effect/1040.frequent_accept.md) | 用于控制是否初始化 net accept 参数。 |
...
| [enable_strict_kernel_release](300.restart-takes-effect/870.enable_strict_kernel_release%20%20.md) | 用于判断是否需要校验 OS 内核。 |
| [max_log_file_size](200.dynamically-effective/1390.max-log-file-size.md) | 用于设置 ODP 中单个日志文件的最大内存限制。 |
...
| [enable_metadb_used](300.restart-takes-effect/610.enable_metadb_used.md) | 用于控制是否开启 MetaDB 访问。 |
...
| [monitor_item_max_idle_period](200.dynamically-effective/1510.monitor-item-max-idle-period%20.md) | 用于设置监控统计信息项目在内存中的保存时间。 |
...
| [qos_stat_item_max_idle_period](200.dynamically-effective/1920.qos-stat-item-max-idle-period.md) | 用于设置 QoS 统计项目在内存中的空闲时间。 |
...
| [limited_pl_update_threshold](200.dynamically-effective/1270.limited-pl-update-threshold.md) | 用于设置在合并状态下，ODP 每秒处理的最大分区位置更新任务数。 |
...
<!-- 下述两个 GitHub 中描述是 unused now, keep it for test,deleted later --> -->
| [max_connections](200.dynamically-effective/1380.max-connections.md) | 用于设置 ODP 允许使用的 fd 最大值。 |
