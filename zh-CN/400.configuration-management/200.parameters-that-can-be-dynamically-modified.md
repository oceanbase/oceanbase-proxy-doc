...
|slug|parameters-that-can-be-dynamically-modified|
...
| `log_file_percentage`                     | 80      | \[0, 100\]                                | ODP 日志文件占用可用空间的百分比阈值。日志占用空间超过 `min(log_dir_size_threshold, 日志文件夹可用空间*log_file_percentage)` 值后进行日志清理，取值为 `0` 时表示忽略此限制。                                                                                    |
| `log_cleanup_interval`                    | 1m   | \[5s, 30d\]                               | 执行清理任务的时间间隔。                                                                                                |
| `log_dir_size_threshold`                  | 64GB    | \[256MB, 1T\]                             | ODP 日志所在目录的最大可用空间阈值。当日志占用空间超过 `min(log_dir_size_threshold, 日志文件夹可用空间*log_file_percentage)` 值后进行日志清理。                                                                                    |
