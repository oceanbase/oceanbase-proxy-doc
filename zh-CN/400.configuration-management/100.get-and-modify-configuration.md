...
<main id="notice" type='explain'>
  ...
  <li>
  <p>修改配置项后的生效方式一般分为动态生效和重启生效两种方式，大部分配置项为动态生效方式，即不需要重启 ODP 就可以生效。</p>
  </li>
  <ul>
</main>
...
3. 执行 `show proxyconfig all [like '%xxx%']` 命令查看多级配置项的值及当前生效范围
   ...
   * `config_level`：配置项的生效级别，有 `LEVEL_GLOBAL`、`LEVEL_CLUSTER`、`LEVEL_TENANT` 和 `LEVEL_VIP` 两种取值。
...
* 在 ODP 运行时，使用 `root@proxysys` 用户登录 ODP 后，执行 `replace into` 命令增加/修改 ODP 配置项。
  ...
  * 支持批量修改，批量修改时，单次修改配置项数量需不大于 200 个，且单条 SQL 长度不超过 1MB。
