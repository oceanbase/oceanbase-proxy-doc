...
|----------|---------|
| 参数类型 |   字符串类型      |
| 默认值   | 无默认值，未配置的情况下表示不压缩数据     |
| 取值范围 | 该配置项的配置格式为：<algorithm_name>:<compression_level>。目前 algorithm_name 仅支持 zlib；compression_level 仅支持 [0-9]，取值为 0 时表示不使用压缩协议和 OceanBase 2.0 协议中的压缩功能。  |
| 是否重启 ODP 生效 | 否  |
