...
| rule.sqlType | 字符串数据，不存在数组中字段的 SQL 全部拦截，触发条件为用户的 SQL 类型与设置值相同，有以下可选值："ALL"（表示全部拦截）、"SELECT"、"UPDATE"、"INSERT"、"REPLACE"、"DELETE"、"MERGE"。 |
...
| inUse  | 布尔值，默认为 `true`，为 `false` 表示该规则不生效。 |
