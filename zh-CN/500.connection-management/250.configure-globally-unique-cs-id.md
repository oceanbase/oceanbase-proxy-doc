...
|slug|configure-globally-unique-client-session-id|
...
* 您当前环境中的 OceanBase 数据库为 V4.2.5 BP4 或之后版本。
...
此处仅为示例，您可将 `proxy_id` 配置为 0~8191 范围中除 2 以外的任意值，不与其他 ODP 重复即可。`proxy_id` 和 `client_session_id_version` 配置项的详细介绍可参见 [proxy_id](../400.configuration-management/200.global-configuration-items/1795.proxy-id.md) 和 [client_session_id_version](../400.configuration-management/200.global-configuration-items/135.client-session-id-version.md)。

### 方法二：ODP 运行中修改配置

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>不推荐使用该方法进行配置，在 ODP 运行时修改 <code>client_session_id_version</code> 和 <code>proxy_id</code> 配置项，容易导致 cs_id 冲突。</p>
</main>
