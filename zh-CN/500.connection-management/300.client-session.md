...
* sys 租户通过 `SHOW PROXYSESSION` 可以查看 ODP 上所有租户的当前客户端连接状态；用户租户则只能查看本租户的当前客户端连接状态。以 sys 租户为例进行操作，示例如下：
...
* sys 租户通过 `SHOW FULL PROCESSLIST` 可以查看 ODP 上所有租户的所有客户端连接状态；用户租户只能查看本租户的所有客户端连接状态。以用户租户为例进行操作，示例如下：
   ...
   +------------+------+-----------+--------------------+-----------+---------+------+--------+-----------------------+--------------+------+
   | 3221636241 | root | testmysql | xx.xx.xx.xx:58600 | oceanbase | Sleep   |   95 | SLEEP  | NULL                  | xx.xx.xx.xx | 2881 |
   | 3221636436 | root | testmysql | xx.xx.xx.xx:58598 | oceanbase | Sleep   | 2795 | SLEEP  | NULL                  | xx.xx.xx.xx | 2881 |
   | 3221647706 | root | testmysql | xx.xx.xx.xx:59362 | test      | Query   |    0 | ACTIVE | show full processlist | xx.xx.xx.xx | 2881 |
   +------------+------+-----------+--------------------+-----------+---------+------+--------+-----------------------+--------------+------+
   ...
   | db                | 执行命令时所处的数据库                                    |
   | Command           | 客户端当前的命令状态                                    |
   | Time              | 当前命令已执行的时长（单位为秒）                |
   | State             | 客户端会话状态，存在如下几个状态：<ul><li>MCS_INIT（初始化）</li><li>MCS_ACTIVE_READER（激活）</li><li>MCS_KEEP_ALIVE（保活）</li><li>MCS_HALF_CLOSE（半关闭）</li><li>MCS_CLOSED（已关闭）</li></ul>                                 |
   | Info              | 当前正在执行或最近一次执行的 SQL 语句 |
   | Ip                | ODP 的 IP 地址  |
   | Port              | ODP 监听的端口号 |
