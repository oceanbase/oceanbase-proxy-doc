...
您可参考本节内容创建 ODP 集群，您可在创建 ODP 集群时部署 ODP，也可在创建 ODP 集群后通过 [添加 OBProxy](#步骤三添加%20ODP) 方式部署 ODP。
...
4. 在 **创建 OBProxy 集群** 页面填写如下信息。
   ...
   2. （可选）配置 **部署 OBProxy**
      ...
      如需在创建 OBProxy 集群时就部署 OBProxy，可在该步骤配置。否则可跳过该步骤，待集群创建成功后，通过 [接管 OBProxy](https://www.oceanbase.com/docs/enterprise-oceanbase-ocp-cn-10000000002100860) 或 [添加 OBProxy](#步骤三添加%20ODP) 向集群中添加 OBProxy。
