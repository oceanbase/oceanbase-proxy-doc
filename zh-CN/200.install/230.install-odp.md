...
2. 执行如下命令安装 ODP 的 RPM 包：
   ...
   <main id="notice" type='explain'>
     ...
     <li>
     <p>ODP 社区版默认安装目录在 <code>/home/admin</code> 下。</p>
     </li>
     </ul>
   </main>
   ...
   ```bash
   -bash-4.2$ sudo rpm -ivh obproxy-ce-4.0.0-20221103165243.el7.x86_64.rpm
   Preparing...                          ################################# [100%]
   Updating / installing...
      1:obproxy-ce-4.0.0-20221103165243.el7 ################################# [100%]
   ```
...
2. 在 ODP 的安装目录下执行如下命令，启动 ODP。
   ...
   1. 方法一：在启动命令中指定 `-r` 参数，命令如下
      ...
      * `-c`：指定 OceanBase 集群。
      ...
      ```bash
      [admin@001 obproxy]$ ./bin/obproxy -p 2883 -r'10.10.10.1:2881;10.10.10.2:2881;10.10.10.3:2881' -o observer_sys_password=$sha1_value,obproxy_sys_password=$sha1_value,enable_cluster_checkout=false -n test -c mycluster
      ```
   ...
   2. 方法二：在启动命令中指定 `obproxy_config_server_url` 参数，命令如下
      ...
      * `obproxy_config_server_url`：指定 Config Server 的访问地址。
      ...
      ```bash
      [admin@001 obproxy]$ ./bin/obproxy -p 2883 -n test -o obproxy_config_server_url='http://xx.xx.xx.xx:8877/obproxy_config',observer_sys_password=$sha1_value,obproxy_sys_password=$sha1_value
      ```
