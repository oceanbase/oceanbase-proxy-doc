...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>建议使用非 root 用户安装 ODP，本文以在 admin 用户下安装 ODP 为例介绍操作。</p>
</main>

1. 执行如下命令添加 sudoers 文件的写权限
   ...
   1. 方法一：在启动命令中指定 `-r` 参数，命令如下
      ...
      * `-c`：指定 OceanBase 集群。
      ...
      <main id="notice" type='notice'>
        ...
        <li>
        <p><code>proxy_id</code> 配置为 1 仅为示例，您可将 <code>proxy_id</code> 配置为 0~8191 范围中除 2 以外的任意值，不与其他 ODP 重复即可。详细介绍可参见 <a href="../400.configuration-management/200.global-configuration-items/1795.proxy-id.md">proxy_id</a>。</p>
        </li>
        </ul>
      </main>
