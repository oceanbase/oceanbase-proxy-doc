...
* OceanBase 集群：详细信息请参考 **OceanBase 数据库文档** 中 [部署数据库](https://www.oceanbase.com/docs/common-oceanbase-database-1000000000033157) 章节。
...
2. 执行如下命令安装 ODP 的 RPM 包：
   ...
   <main id="notice" type='explain'>
     ...
     <ul>
     <li>
     <p>您可联系技术人员获取最新的 ODP RPM 包，也可在 <a href="https://www.oceanbase.com/softwarecenter">OceanBase 软件下载中心</a> 页面搜索 <b>OceanBase 数据库代理</b>，根据实际情况下载对应的 ODP RPM 包。</p>
     </li>
     <li>
     <p>ODP 企业版默认安装目录在 <code>/opt/taobao/install</code> 下，社区版默认安装目录在 <code>/home/admin</code> 下。</p>
     </li>
     </ul>
   </main>
...
3. （可选）创建到 ODP 的软链接
   ...
   创建到 ODP 的软链接可以隐藏掉版本信息，方便后期操作。此处以企业版 ODP 为例介绍创建软链接操作。
...
* 在启动命令中指定 `-r` 参数来指定 OceanBase 集群的 RootServer 信息
  ...
  该启动方式不需要额外配置，一般用于开发调试阶段。通过该方法启动 ODP 后，当 OceanBase 数据库中 Root Service 列表有变动时，需手动更新 `rootservice_list` 配置项，并重启 ODP 使配置生效。
...
2. 在 ODP 的安装目录下执行如下命令，启动 ODP。
   ...
   1. 方法一：在启动命令中指定 `-r` 参数，命令如下
      ...
      * `-c`：指定 OceanBase 集群。
      ...
      ```bash
      [admin@001 obproxy]$ ./bin/obproxy -p 2883 -r'10.10.10.1:2881;10.10.10.2:2881;10.10.10.3:2881' -o observer_sys_password=$sha1_value,obproxy_sys_password=$sha1_value,enable_cluster_checkout=false,proxy_id=1,client_session_id_version=2 -n test -c mycluster
      ```
      ...
      <main id="notice" type='notice'>
        ...
        <li>
        <p><code>proxy_id</code> 配置为 1 仅为示例，您可将 <code>proxy_id</code> 配置为 0~8191 范围中除 2 以外的任意值，不与其他 ODP 重复即可。详细介绍可参见 <a href="../400.configuration-management/200.dynamically-effective/1795.proxy-id.md">proxy_id</a>。</p>
        </li>
        </ul>
      </main>
   ...
   2. 方法二：在启动命令中指定 `obproxy_config_server_url` 参数，命令如下
      ...
      * `obproxy_config_server_url`：指定 Config Server 的访问地址。
      ...
      ```bash
      [admin@001 obproxy]$ ./bin/obproxy -p 2883 -n test -o obproxy_config_server_url='http://xx.xx.xx.xx:8877/obproxy_config',observer_sys_password=$sha1_value,obproxy_sys_password=$sha1_value,proxy_id=1,client_session_id_version=2
      ```
