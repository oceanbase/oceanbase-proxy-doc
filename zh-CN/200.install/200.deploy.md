...
* 部署方式
  ...
  不同部署方式背后反映的是 RT 和资源抢占，部署方式的不同极大地影响着性能。
...
下图是专有云很常见的部署形态，其中 LB 为负载均衡，您需自行部署并配置：

![图二](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odp/V4.2.1/zh-CN/200.install/1.png)

## 与 OBServer 节点分开部署

您可选择部署在每一个需要访问数据库的应用服务器上，也可选择独立部署。

* 部署在每一个需要访问数据库的应用服务器上时，应用程序直接连接部署在同一台服务器上的 ODP，所有请求会由 ODP 发送到合适的 OceanBase 数据库服务器。
