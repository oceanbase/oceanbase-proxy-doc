...
4. 在 **创建 OBProxy 集群** 页面填写如下信息。
   ...
   2. （可选）配置 **部署 OBProxy**
      ...
      如需在创建 OBProxy 集群时就部署 OBProxy，可在该步骤配置。否则可跳过该步骤，待集群创建成功后，通过 [接管 OBProxy](https://www.oceanbase.com/docs/community-ocp-cn-10000000001834160) 或 [添加 OBProxy](#步骤三：添加%20ODP) 向集群中添加 OBProxy。
