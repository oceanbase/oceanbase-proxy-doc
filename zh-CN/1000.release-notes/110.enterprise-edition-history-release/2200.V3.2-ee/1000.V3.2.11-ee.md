...
* 版本号：V3.2.11 BP4 hotfix5

* RPM 版本号：obproxy-3.2.11.0-20240704210949
...
* 发布时间：2024 年 01 月 16 日

* 版本号：V3.2.11 BP4 hotfix3

* RPM 版本号：obproxy-3.2.11.0-20240116145744

### 缺陷修复

* 修复 C# 驱动 SSL 连接失败的问题。
* 修复 Visual Studio for Windows 连接不上 OceanBase 数据库的问题。
...
* 版本号：V3.2.11 BP4 hotfix2

* RPM 版本号：obproxy-3.2.11.0-20231220120421
...
* 修复 <code>show columns from `db_name`.`table_name`</code> 语句识别问题。
* 修复 Range 分区中，没有正确处理范围为 MAX 的分区范围的问题。
* 修复 ODP 在连接数超限后，只返回 error 包，但实际并不执行 close，客户端也不执行 close，导致连接泄漏的问题。
* 修复 ODP 读取 conn attr 需要兼容 atto 为 0 的问题。
