...
|slug|odp-enterprise-edition-v4-2-3|
...
* 支持全局唯一 Client Session ID，支持 ODP 到 OBServer 节点的协议压缩功能，增强链路能力。
...
详细配置方法可参见 [配置全局唯一的 Client Session ID](../../../500.connection-management/250.configure-globally-unique-cs-id.md)。
...
| mem_leak_check_class_name | 新增  | 用于设置 `show proxymemory objpool` 命令需要监控的对应类名（`free_list_name`），取值为 `ALL` 时代表全部监控（建议只在 DEBUG 时开启）。 |
| protocol_diagnosis_level | 新增  | 用于控制是否开启协议诊断，以及是否将诊断信息输出到日志中。默认值为 0，表示关闭协议诊断。 |
| compression_algorithm | 新增  | 用于控制压缩协议和 OceanBase 2.0 协议的压缩级别，取值范围为 zlib:[0-9]，取值为 `zlib:0` 时表示不使用压缩协议和 OceanBase 2.0 协议中的压缩功能。 |
