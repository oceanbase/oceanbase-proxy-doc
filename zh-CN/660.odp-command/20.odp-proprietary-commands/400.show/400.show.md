...
| PROXYROUTE GLOBALINDEX  | 展示 ODP 中的全局索引信息。缓存信息的存在时间可通过修改 [partition_location_expire_relative_time](../../../400.configuration-management/200.global-configuration-items/1720.partition-location-expire-rela.md) 配置项控制。  |
...
| PROXYROUTE RPCCTX  | 展示 ODP 中保存的登录相关信息，可通过该命令验证 ODP 对登录用户是否支持分布式能力。不支持通过配置项清理，同时也不建议清理查看到的登录信息，否则可能会导致用户无法登录。  |
