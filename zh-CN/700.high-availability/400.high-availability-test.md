...
|slug|high-availability-testing|

# 高可用测试

您也可以通过测试感受一下 ODP 的高可用，当然，我们无法在跑业务压测时把机房电源关闭，但对于 OceanBase 数据库所提到的 RTO 表现，您可用 sysbench 工具（下面测试基于 OceanBase 4.0 版本）进行测试。方法非常简单，先将 sysbench 压测正常跑起来，然后利用 kill -9/-19/-18 操作 OBserver 或者 ODP 进程，观察 TPS 的变化。
