...
![示例](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odp/V4.2.1/zh-CN/700.high-availability/1.png)
...
1. 在客户端中使用 root 用户，通过 ODP 代理登录集群的 sys 租户。
   ...
   此处是以 ODP 机器 IP 为 `10.10.10.1`，集群名称为 `obdemo`，连接端口为 `2883` 为例，通过 OBClient 连接集群，您需根据实际情况进行修改。
...
2. 执行如下命令查看黑名单信息。
   ...
   * 仅指定 `clustername` 时，可显示指定集群的黑名单信息，指定 `clustername` 时需使用英文单引号或双引号引起来。
...
<main id="notice" type='explain'>
  ...
  <ul>
  <li>
  <p><code>dead_congested</code> 和 <code>alive_congested</code> 两个字段分别对应黑名单和活不可用黑名单，这两个字段中任意一个状态为 <code>1</code> 时，表示该 OBServer 节点已被加入黑名单。</p>
  </li>
  <li>
  <p>如果机器被加入活不可用黑名单，ODP 在 <code>congestion_retry_interval</code> 时间内不会转发任何请求给该机器，并在 <code>congestion_retry_interval</code> 时间后发起重试，并且该 OBServer 节点在 <code>min_keep_congestion_interval</code> 时间内不能被洗白。</p>
  </li>
  </ul>
</main>
...
![转发前重试](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/odp/V4.2.1/zh-CN/700.high-availability/2.png)
...
ODP 会在数据路由阶段选择 OBServer1 节点，之后进行容灾管理检查，发现 OBServer1 节点在状态黑名单中，此时就需要进行路由重试。通过 [租户内路由](../600.data-routing/400.intra-tenant-routing/100.overview-of-routing.md)，从租户的机器列表中选择机器路由，然后再进行容灾检查，直到容灾检查通过才进行转发。
