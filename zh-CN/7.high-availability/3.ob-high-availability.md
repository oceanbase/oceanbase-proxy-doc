...
其中前六个状态通过访问 OBServer 的视图可以获得，ODP 会周期性（20s/次）从 `DBA_OB_SERVERS` 和  `DBA_OB_ZONES` 表中获取集群的机器状态信息，ODP 会通过 status、start_service_time、stop_time 定义 OBServer 状态。而 `DETECT_ALIVE` 和 `DETECT_DEAD` 状态则通过 ODP 的探测机制获得。
