...
* 日志格式
  ...
  ```bash
  日志打印时间,逻辑租户名,TraceId,RpcId,逻辑库名,物理库信息(cluster:tenant:database),数据库类型(OB/RDS),逻辑表名,物理表名,SQL 命令,SQL 类型(CRUD),执行结果(success/failed),错误码(success 时为空),SQL,执行总耗时(us),ODP 处理时间,链接建立时间,OBServer 节点执行时间,当前线程名,无需关注字段
  ```
...
对于慢 SQL，在 obproxy.log 中也会有记录，关键字是 `Slow Query`，obproxy.log 中记录的信息更加详细，如执行 SQL 语句 `select sleep(3) from dual`，搜索 obproxy.log 日志将得到如下信息：
...
cmd_time_stats={
    ...
    server_request_write_time_us=32,  // ODP 向目标 OBServer 节点发送请求包的耗时
    server_process_request_time_us=3039883,   // 目标 OBServer 节点执行 SQL 的耗时
    server_response_read_time_us=67,   // ODP 从目标 OBServer 节点读取响应包的耗时
...
`slow_query_time_threshold` 配置项的详细介绍可参见 [slow_query_time_threshold](../400.configuration-management/300.multi-level-configuration-items/2350.slow-query-time-threshold.md)。
