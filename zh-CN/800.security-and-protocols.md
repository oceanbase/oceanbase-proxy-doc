...
* enable_ob_protocol_v2：判断是否使用 OceanBase 2.0 协议，详细介绍可参见 [enable_ob_protocol_v2](400.configuration-management/200.dynamically-effective/660.enable-ob-protocol-v2.md)。

* enable_compression_protocol：判断是否使用压缩协议，详细介绍可参见 [enable_compression_protocol](400.configuration-management/200.dynamically-effective/490.enable-compression-protocol.md)。

enable_ob_protocol_v2 的生效范围高于 enable_compression_protocol，两个配置项都为 false 则表示使用普通协议。在私有云环境使用压缩协议，在公有云使用 OceanBase 2.0 协议。
...
压缩协议和 OceanBase 2.0 协议都会对数据进行校验，保证数据正确性，之前也曾出现过数据错误的情况。除了数据校验，压缩协议可以节约带宽，但这是通过消耗 CPU 换取来的。

### 功能特性

目前新的特性都是基于 OceanBase 2.0 协议开发，后续 OceanBase 2.0 协议将会成为主流。OceanBase 2.0 协议格式如下：
