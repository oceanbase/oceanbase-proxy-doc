...
* OceanBase 2.0 协议：自研协议，支持更多特性开发
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>在 ODP 4.x 版本和 OceanBase 数据库 4.x 版本中不建议关闭 OceanBase 2.0 协议，否则会有功能缺失。</p>
  </main>

有以下两个配置项控制协议使用：

* server_protocol：控制 ODP 与 OceanBase 数据库之间的协议类型，详细介绍可参见 [server_protocol](400.configuration-management/200.global-configuration-items/2119.server-protocol.md)。
...
压缩协议和 OceanBase 2.0 协议都会对数据进行校验，保证数据正确性。
