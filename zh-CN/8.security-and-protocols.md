...
ODP 有两个特殊的账号：`root@proxysys` 和 `proxyro@sys`，这里 `root@proxysys` 是 ODP 的管理员账号，`proxyro@sys` 是 ODP 访问 OceanBase 数据库的账号。这两个账户都支持修改密码，其中 `root@proxys` 的密码对应配置项 `obproxy_sys_password`，`proxyro@sys` 的密码对应配置项 `observer_sys_password`，为了实现 proxyro 的平滑改密（不重启），最新 3.x 版本增加了 `observer_sys_password1`。
