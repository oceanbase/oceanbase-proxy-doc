...
本文主要介绍分区表达式的支持情况以及如何使用和验证是否路由成功。
...
* 分区表达式路由为 ODP V4.2.3 迭代功能，使用该功能时需确保您所使用的 ODP 为 V4.2.3 或之后版本。
...
本节介绍分区键和默认值中支持的表达式，ODP 支持基本的四则运算（包括 `+`、`-`、`*`、`/`），详细使用可参加下文 [基本表达式用法](#基本表达式用法)。除此之外还支持字符串表达式、数字计算表达式和日期表达式。本节将分别介绍 ODP 对各表达式的支持情况，详细的使用介绍可参见下文 [使用说明](#使用说明)。

#### 字符串表达式

ODP 对字符串表达式的支持情况如下表，具体的用法可参见 [字符串表达式](200.string-expression.md)。
...
ODP 对数字计算操作相关表达式的支持情况如下表，具体的用法可参见 [数字计算操作](300.numerical-calculation-operation.md)。
...
ODP 对日期计算操作相关表达式的支持情况如下表，具体的用法可参见 [日期计算操作](400.date-calculation-operation.md)。
