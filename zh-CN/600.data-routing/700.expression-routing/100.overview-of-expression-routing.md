...
|slug|expression-routing-overview|
...
OceanBase 数据库是分布式数据库, 每个表甚至每个表的不同分区都可能存放在不同的机器上，若要对表进行读写，就必须先要定位到数据所属的表或是分区的位置。尽管任何一台 OBServer 节点均拥有路由转发和本地执行的功能，但实际场景中，SQL 请求没有转发到最恰当的 OBServer 节点将严重影响性能和整体吞吐量。
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>自 ODP V4.3.4 起，支持分区表达式中存在函数进行路由。</p>
</main>
...
| TO_DAYS               | 将给定的日期转换为自公元 0 年以来的天数（整数） | MySQL   |
| MONTH                 | 从给定的日期中提取月份                      | MySQL    |
