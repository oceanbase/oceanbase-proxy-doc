...
* 强读路由：客户端发送分区表强读请求 SQL，SQL 语句提供分区键值、表达式或分区名，ODP 会将语句准确路由到数据所在分区的主副本 OBServer 节点执行。详细介绍请参见 [强读路由](300.strong-read-partition-table-routing.md)。
...
* 基于权重的负载均衡：通过 ODP 配置项（weadread_weight_zone）为 OceanBase 集群中的各个 Zone 设置权重，ODP 在路由时会基于权重进行负载均衡，详细介绍请参见 [基于权重的负载均衡](1100.weight-based-load-balancing.md)。
