...
* TARGET_REPLICA_TYPE_WITH_LEADER：指定副本路由，主副本与备副本优先级相同。备副本的类型由 `route_target_replica_type` 决定。

* TARGET_REPLICA_TYPE_FOLLOWER_FIRST：指定副本路由，优先选择备副本，当所有备副本不可用时会选择主副本。备副本的类型由 `route_target_replica_type` 决定。

* TARGET_REPLICA_TYPE_FOLLOWER_ONLY：指定副本路由，只会选择备副本，当备副本都不可用时，断开和客户端的连接。备副本的类型由 `route_target_replica_type` 决定。

* WEAKREAD_WEIGHT_LOAD_BALANCE：基于权重进行负载均衡路由，可通过配置 `weakread_weight_zone` 为集群中的各个 Zone 设置权重。

`route_target_replica_type` 的详细介绍可参见 [route_target_replica_type](../../400.configuration-management/300.multi-level-configuration-items/2040.route_target_replica_type.md)，`weakread_weight_zone` 的详细介绍可参见 [weakread_weight_zone](../../400.configuration-management/300.multi-level-configuration-items/2600.weakread-weight-zone.md)。

<main id="notice" type='notice'>
  <h4>注意</h4>
  <ul>
  <li><p>当 <code>route_target_replica_type</code> 配置为 <code>ColumnStore</code> 时，ODP 只会将请求路由到列存副本，不会路由到主副本。</p></li>
  <li><p>只读 Zone 相关路由策略已经废弃，请勿使用。</p></li>
  </ul>
</main>
