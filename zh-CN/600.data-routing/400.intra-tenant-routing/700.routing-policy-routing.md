...
* TARGET_REPLICA_TYPE_WITH_LEADER：优先选择主副本，备副本优先级相同。

* TARGET_REPLICA_TYPE_FOLLOWER_FIRST：会选LEADER，但FOLLOWER优先级更高。

* TARGET_REPLICA_TYPE_FOLLOWER_ONLY：不会路由到主副本。

当配置项 `proxy_route_policy` 取值为后三种（`TARGET_REPLICA_TYPE_` 开头）路由策略时，可通过配置 `route_target_replica_type` 指定备副本类型，`route_target_replica_type` 的详细介绍可参见 [route_target_replica_type](../../400.configuration-management/300.multi-level-configuration-items/2040.route_target_replica_type.md)。
