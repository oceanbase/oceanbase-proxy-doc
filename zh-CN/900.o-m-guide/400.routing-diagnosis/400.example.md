...
|slug|routing-diagnosis-examples|
...
输出如下，可以看到 ROUTE_INFO 提示了指定 IP 路由（USE_COMMENT_TARGET_DB），首先选择了 10.10.10.1:5011，之后查询黑白名单（CONGESTION_CONTROL 诊断点），发现 `entry_exist` 值为 false，表明没有查询到相关 OBServer 节点的黑名单信息，ODP 则默认为该 OBServer 节点状态为 DEAD_CONGESTED，并输出 HANDLE_RESPONSE 诊断点表明该 OBServer 节点的状态。ODP 继续重试其他的指定 IP 地址，同时显示重试次数。
