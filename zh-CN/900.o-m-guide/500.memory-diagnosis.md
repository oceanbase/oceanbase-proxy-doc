...
使用内存泄漏诊断功能前，您需确认环境中部署的 ODP 为 V4.2.3 或之后版本。
...
2. 执行如下命令设置功能相关配置项
   ...
   设置 `mem_leak_check_mod_name` 配置项，排查内存泄漏问题时推荐配置为 `ALL`。该配置项的详细介绍可参见 [mem_leak_check_mod_name](../400.configuration-management/200.global-configuration-items/1437.mem-leak-check-mod-name.md)。

   ```shell
   obclient> alter proxyconfig set mem_leak_check_mod_name='ALL';
   ```

   设置 `mem_leak_check_class_name` 配置项，排查内存泄漏问题时推荐配置为空。该配置项的详细介绍可参见 [mem_leak_check_class_name](../400.configuration-management/200.global-configuration-items/1435.mem-leak-check-class-name.md)。
...
5. 在 ODP 的安装目录下，执行 addr2line 命令查看内存分配位置，进而确定内存泄漏原因
...
`proxy_mem_limited` 配置项的详细介绍可参见 [proxy_mem_limited](../400.configuration-management/200.global-configuration-items/1830.proxy-mem-limited.md)。
