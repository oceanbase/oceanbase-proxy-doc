...
* show trace 功能的诊断信息会在 ODP 和 OceanBase 数据库之间交互，您可以看到完整的路径记录信息。
...
使用 show trace 时需使用上述正确版本，并开启 OceanBase 2.0 协议，具体方法如下。

1. ODP 中可通过配置项 :::bubble enable_ob_protocol_v2 # 默认为 true，控制 ODP 和 OceanBase 数据库之间是否开启 OceanBase 2.0 协议:::控制 ODP 和 OceanBase 数据库的交互协议。您可通过如下两种方法配置。

   * 方法一：ODP 进程启动时通过 `-o` 参数配置。

      ```shell
      cd /opt/taobao/install/obproxy
      ./bin/obproxy -o enable_ob_protocol_v2=true
      ```
...
3. JDBC 可通过 URL 控制开启，在 JDBC URL 中添加 `useOceanBaseProtocolV20=true`。

### 步骤二：开启 show trace 功能并执行目标 SQL

1. 当各个组件的版本支持 show trace 功能，且都开启了 OceanBase 2.0 协议时，通过客户端连接 OceanBase 数据库后执行如下命令开启 session 级别 show trace。
