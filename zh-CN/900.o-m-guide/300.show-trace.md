...
* 当使用正确的组件版本时，可以通过 session 级别功能开关开启 session 级别的 show trace 能力，开启后的每一条 SQL、驱动、ODP 和 OceanBase 数据库都将记录 SQL 的交互信息。SQL 执行结束后，您可直接执行 show trace 命令得到上一条执行 SQL 记录的诊断信息。
