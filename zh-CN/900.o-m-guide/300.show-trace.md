...
1. ODP 中可通过配置项 enable_ob_protocol_v2_with_client 和 enable_ob_protocol_v2 分别控制 ODP 和驱动以及 ODP 和 OceanBase 数据库的交互协议。您可通过如下两种方法配置。
...
1. 当各个组件的版本支持 show trace 功能，且都开启了 OceanBase 2.0 协议，开启全链路诊断。此时，通过客户端连接 OceanBase 数据库后执行如下命令开启 session 级别 show trace。
