...
2. 在 OBProxy 的安装目录下执行如下命令，启动 OBProxy。
   ...
   > **说明**
   >
   > 在启动 OBProxy 前，如果需要了解 OBProxy 的启动参数有哪些，可以通过 `./bin/obproxy -h` 命令查看。
   ...
   1. 在启动命令中指定 `-r` 参数，命令如下：
      ...
      * `-p` ：指定 OBProxy 监听的端口号，客户端通过 MySQL 连接该端口访问 OceanBase 数据库。仅在第一次启动时需要指定该参数，后续续日常启动或升级等均不需要添加此参数。
...
3. 启动后，执行以下命令，查看 obproxy 进程是否存在:
